...
class Benchmark(object):
    def __init__(self, order_book_id, env):
    ...
    def annualized_returns(self):
        ...
        return (self.total_returns + 1) ** (DAYS_CNT.TRADING_DAYS_A_YEAR / date_count) - 1


class BackTestPriceSeriesBenchmark(Benchmark):
    def __init__(self, order_book_id, env):
        super(BackTestPriceSeriesBenchmark, self).__init__(order_book_id, env)
        ...
        self._env.event_bus.prepend_listener(EVENT.AFTER_TRADING, self._on_after_trading)

    def _on_system_init(self, __):
        bar_count = len(self._env.config.base.trading_calendar) + 1
        end_date = self._env.config.base.end_date
        close_series = self._env.data_proxy.history_bars(
...
class RealTimePriceSeriesBenchmark(Benchmark):
    def __init__(self, order_book_id, env):
        super(RealTimePriceSeriesBenchmark, self).__init__(order_book_id, env)
