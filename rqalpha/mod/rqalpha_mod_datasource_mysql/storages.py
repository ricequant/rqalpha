...
from rqalpha.mod.rqalpha_mod_datasource_mysql.models import session
...
class DayBarStore(AbstractDayBarStore):
    ...
    def get_date_range(self, order_book_id):
        try:
            data = pd.read_sql(session.query(self.model).filter_by(code=order_book_id).statement, session.bind)
            return min(data['datetime'].astype(np.uint64).values), max(data['datetime'].astype(np.uint64).values)
        except KeyError:
