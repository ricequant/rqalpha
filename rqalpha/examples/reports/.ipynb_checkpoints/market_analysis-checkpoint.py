...
from numba import jit, int64, int32, np
...
class MarketView(object):
    ...
    def panel_data(date=dt.datetime.now(), count=120, stock_min_list_days=180):
        ...
        instrs = [instrument for instrument in instruments if instrument.listed_date < (date - dt.timedelta(days=stock_min_list_days)) and instrument.status=='Active']
        
        datas = []
        for ins in instrs:
            print(ins.order_book_id)
            data = data_proxy.history_bars(ins.order_book_id, dt=date, bar_count=count, frequency='1d', field=['open', 'high', 'low', 'close', 'volume', 'total_turnover'])
            data['order_book_id'] = ins.order_book_id
            data['industry_name'] = ins.industry_name
            data['board_type'] = ins.board_type
            datas.append(data)
        df = pd.concat(datas)
        return df
    ...
    def index_change(date=dt.datetime.now()):
        ...
        for order_book_id in zhishu_list:
            data = data_proxy.history_bars(order_book_id, dt=date, bar_count=11,
                                           field=['close', 'volume', 'total_turnover'])
            data['order_book_id'] = order_book_id
            panel = pd.concat([panel, data], axis=1)
        
        s_close = panel['close'].iloc[-1]
        s_return1 = ((panel['close'].iloc[-1] / panel['close'].iloc[-2]) - 1) * 100
        s_return3 = ((panel['close'].iloc[-1] / panel['close'].iloc[-4]) - 1) * 100
        s_return5 = ((panel['close'].iloc[-1] / panel['close'].iloc[-6]) - 1) * 100
        s_volume = ((panel['volume'].iloc[-1] / panel['volume'].iloc[-2]) - 1) * 100
        s_volume3 = ((panel['volume'].iloc[-3:].sum() / panel['volume'].iloc[-6:-3].sum()) - 1) * 100
        s_volume5 = ((panel['volume'].iloc[-5:].sum() / panel['volume'].iloc[-10:-5].sum()) - 1) * 100
        s_money = ((panel['total_turnover'].iloc[-1] / panel['total_turnover'].iloc[-2]) - 1) * 100
        s_money3 = ((panel['total_turnover'].iloc[-3:].sum() / panel['total_turnover'].iloc[-6:-3].sum()) - 1) * 100
        s_money5 = ((panel['total_turnover'].iloc[-5:].sum() / panel['total_turnover'].iloc[-10:-5].sum()) - 1) * 100
        
        df = pd.DataFrame({'指数值': s_close, '涨跌幅%': s_return1, '3日涨跌%': s_return3, '5日涨跌%': s_return5, \
                           '1日量变%': s_volume, '3日量变%': s_volume3, '5日量变%': s_volume5, '1日额变%': s_money, \
                           '3日额变%': s_money3, '5日额变%': s_money5}, index=s_close.index)
    ...
    @staticmethod
    def 涨跌统计(panel, count=14):
        s_涨停 = (panel['close'].values == panel['high_limit'].values).sum(axis=1)
        s_炸板成功 = ((panel['high'].values == panel['high_limit'].values) & (
            (panel['close'].values < panel['high_limit'].values))).sum(axis=1)
        s_up7p = (panel['close'].values > panel['pre_close'].values * 1.07).sum(axis=1)
        s_up57 = ((panel['close'].values > panel['pre_close'].values * 1.05) & (
                panel['close'].values <= panel['pre_close'].values * 1.07)).sum(axis=1)
        s_up35 = ((panel['close'].values > panel['pre_close'].values * 1.03) & (
                panel['close'].values <= panel['pre_close'].values * 1.05)).sum(axis=1)
        s_up03 = ((panel['close'].values > panel['pre_close'].values * 1.00) & (
                panel['close'].values <= panel['pre_close'].values * 1.03)).sum(axis=1)
        s_dn03 = ((panel['close'].values <= panel['pre_close'].values * 1.00) & (
                panel['close'].values >= panel['pre_close'].values * 0.97)).sum(axis=1)
        s_dn35 = ((panel['close'].values < panel['pre_close'].values * 0.97) & (
                panel['close'].values <= panel['pre_close'].values * 0.95)).sum(axis=1)
        s_dn57 = ((panel['close'].values < panel['pre_close'].values * 0.95) & (
                panel['close'].values <= panel['pre_close'].values * 0.93)).sum(axis=1)
        s_dn7p = ((panel['close'].values < panel['pre_close'].values * 0.93) & (
                panel['close'].values > panel['pre_close'].values * 0.90)).sum(axis=1)
        s_跌停 = (panel['low'].values == panel['low_limit'].values).sum(axis=1)
        df = pd.DataFrame({'涨停': s_涨停, \
                           '+7-10': s_up7p, '+5-7': s_up57, '+3-5': s_up35, '+0-3': s_up03, \
                           '-0-3': s_dn03, '-3-5': s_dn35, '-5-7': s_dn57, '-7-10': s_dn7p, '跌停': s_跌停, '炸板': s_炸板成功 \
                           }, index=panel['close'].index)
        df = df.sort_index(ascending=False)
        df1 = df.iloc[:count]
        start_date = df1.index[-1]
        end_date = df1.index[0]
        df1.index = df1.index.strftime("%Y%m%d")
        cm = sns.light_palette("green", as_cmap=True)
        return df1.T.style.background_gradient(cmap=cm, axis=1). \
            set_caption(
    ...
    @staticmethod
    def 涨停记录(panel, filter_st=False):
        """日期，股票代码，第几个涨停 """
        # 涨停
        arr_up = ((panel['close'].values == panel['high_limit'].values) & (panel['paused'].values == 0)).astype(int)
        # ST股票涨跌停5%
        st = panel['close'].columns[(panel['high_limit'].values < panel['low_limit'].values * 1.2).any(axis=0)]
        # 构建二维数组，1代表涨停，-1代表停牌，0 代表其他
        arr_paused = np.where(panel['paused'].values > 0, -1, 0)
        arr_cb = np.where(arr_up > 0, arr_up, arr_paused)
        # 涨停累加DataFrame
        df = pd.DataFrame(MarketView.lianban(arr_up), index=panel['close'].index, columns=panel['close'].columns)
        if filter_st:
            st_columns = panel['close'].columns[
                (panel['high_limit'].values < panel['low_limit'].values * 1.2).any(axis=0)]
...
data = MarketView.panel_data()
data['pre_close'] = data.close.shift(1)
data['high_limit'] = data.apply(lambda row: round_up(row.pre_close*1.1,2) if row.board_type=='MainBoard' else round_up(row.pre_close*1.2,2))
data['low_limit'] = data.apply(lambda row: round_up(row.pre_close*0.9,2) if row.board_type=='MainBoard' else round_up(row.pre_close*0.8,2))
