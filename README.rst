...
将国内最受欢迎的米筐自研开源策略引擎 RQAlpha打通期货实盘，实现策略的全自动化交易， 托管运行在云端，无需为策略的稳定运行和硬件支持担忧。

.. image:: https://raw.githubusercontent.com/ricequant/rqalpha/master/docs/source/_static/rqpro.png

策略可进行自动化实盘交易是最不可或缺的一环，RQPro终端期货实盘目前合作的期货商有东亚期货、安粮期货和中信期货，支持的期货商仍在扩充中，以期尽可能满足您的期货商选择。RQPro实盘交易的核心优势如下：

* 基于RQAlpha引擎，拥有强劲的性能，从回测至实盘无需更改代码，支持回测、模拟仿真和实盘交易一体化。
* 支持十年全品种（商品、国债、股指）、多频率（日、分钟、Tick）期货历史数据。
* 实时数据直连CTP，提供实时tick行情，历史数据自动清洗入库。
* 支持期货夜盘，符合看穿式监管合规要求。
* 实盘策略加密后运行在云端，兼顾策略私密性和硬件管理问题。
* 云端采用K8S技术支持多点备份，保证低延迟和稳定性。
* 包含专为实盘打造的仓位管理系统，支持手工增删成交来调整策略的仓位。可以在对账出现差异或因在其他系统中经由同一资金账号交易后的成交回报不同步，调整仓位使得策略仓位与实际仓位一致。

目前的云端实盘交易功能仅支持期货交易，如需股票程序化交易，请等待后续中泰证券量化交易终端下载。期货实盘交易、在米筐主站注册账号即可在终端内使用。更多详细可见 `《期货实盘说明》 <https://assets.ricequant.com/welcome/期货实盘说明.09b0d7b5.pdf>`_ ，欢迎致电0755-22676337联系我们咨询云端期货实盘信息。
