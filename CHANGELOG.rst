...
- 策略运行结束后输出的结果中加入部分超额收益和周度风险


4.5.1
==================

- :code:`Instrument` 对象增加 `trading_code` 字段，意味该标的在交易所的代码
- 修复 :code:`get_pit_financials_ex` 的异常行为
- :code:`get_positions` 接口不再返回数量为 0 的持仓对象
- 修复了在分红未到账时平仓会导致分红金额始终不到账的问题
- 修复了 :code`get_open_orders` 取不到集合竞价阶段挂单的问题及其导致的冻结资金问题
...
- 修复了调取 :code:`history_bars` 获取到错误的复权价错误的问题
